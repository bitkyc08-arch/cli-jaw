...
export async function cliHandler(args: string[], ctx: CliCommandContext): Promise<SlashResult> {
    ...
    const allowed = Object.keys((settings["perCli"] as Record<string, unknown> | undefined) || {})
        .filter((cli): cli is typeof CLI_KEYS[number] => CLI_KEYS.includes(cli as typeof CLI_KEYS[number]));
    const fallbackAllowed: readonly string[] = allowed.length ? allowed : DEFAULT_CLI_CHOICES;
