...
- Slack connection environment variables are runtime-only and exclusive: Settings exposes only their names, connection fields are read-only, connection writes/reset return `409`, CLI setup paths refuse mixed file credentials, and persistence strips environment-managed fields so secrets never enter `settings.json`.
