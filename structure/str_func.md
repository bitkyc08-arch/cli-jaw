...
│   │       ├── activity.ts   ← Native Activity disclosure, terminal status and bounded preview descriptor (105L)
│   │       ├── activity-answer.ts ← Full authoritative answer and empty/absent receipts (26L)
...
│       ├── chat.ts           ← 터미널 채팅 TUI (3모드, locale bootstrap, refreshInfo, active model 표시, no-arg `/model`·`/cli` selector intercept, transcript 축적, overlay wiring, batched key tokenization, settings snapshot, 372L)
