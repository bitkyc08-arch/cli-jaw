...
function adoptReplayModel(ctx: TuiContext, replay: ActivityReplay, model: ActivityState): void {
    if (!sameIdentity(ctx.activityIdentity ?? ctx.activitySettlementIdentity, model.identity)) return;
...
export async function restoreActiveActivity(ctx: TuiContext): Promise<void> {
    ...
    try {
        await replay.restore(async signal => {
            ...
            const read = (path: string, readSignal: AbortSignal) => apiJson(ctx, path, { signal: readSignal });
            ...
            if (seed.scope !== identity.scope || tail.scope !== identity.scope) throw new Error('activity_scope_changed');
        ...
        for (const event of result.events) {
            ...
            if (event.kind === 'turn-end' && ctx.displayMode === 'line') {
                for (const item of turns(ctx)) process.stdout.write(closeActivityLinear(item));
                writeActivityAnswer(ctx.store.transcript, activityKey(event), process.stdout.columns || 80, text => process.stdout.write('\n' + text));
            }
        }
