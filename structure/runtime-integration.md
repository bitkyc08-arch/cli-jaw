...
Tools/toolOutput are available on native main; public native workers and approval/question/image/subagent capabilities remain disabled, and pending tool decisions are denied. Foreground-only hooks reject unsupported background modes without promising an OS sandbox. Claude print and claude-e remain unchanged. SDK authentication follows the official API/cloud setup; no claude.ai login flow, credential copying or subscription entitlement is added. Current integration qualification distinguishes real pinned SDK/owned simulated CLI from historical provider evidence and does not claim a fresh live-provider run or visual Activity acceptance.
