...
export interface AcpReplacementIO {
...
export type AcpReplacementResult = { accepted: true; epoch: number }
...
export class AcpReplacementError extends Error {
    constructor(readonly stage: 'cancel' | 'dispatch', cause: unknown) {
        super(`acp_replacement_${stage}_failed`, { cause });
        this.name = 'AcpReplacementError';
    }
}
...
export class AcpReplacement {
    ...
    private failure: AcpReplacementError | null = null;

    constructor(private readonly io: AcpReplacementIO) {}
    ...
    async first(prompt: RuntimePrompt): Promise<void> {
        ...
        if (this.stopped) throw new Error('acp_control_stopped');
        if (this.firstRequested) throw new Error('acp_control_already_started');
        const captured = copyPrompt(prompt);
        this.firstRequested = true;
        return this.serial(async () => {
            if (this.stopped) throw this.failure ?? new Error('acp_control_stopped');
            this.epoch = 1;
            await this.dispatch(captured, this.epoch);
        });
    }

    async replace(prompt: RuntimePrompt): Promise<AcpReplacementResult> {
        if (this.failure) throw this.failure;
    ...
    private noStart(reason: Extract<AcpReplacementResult, { accepted: false }>['reason']): AcpReplacementResult {
    ...
    private fail(stage: AcpReplacementError['stage'], cause: unknown): AcpReplacementError {
        if (this.failure) return this.failure;
        const failure = new AcpReplacementError(stage, cause);
        this.failure = failure;
        this.stopped = true;
        this.pendingIntent = 0;
        this.latestIntent++;
        try { this.io.retire(failure); }
        catch (error) { failure.cause = new AggregateError([cause, error], 'acp_retirement_failed'); }
        return failure;
    }
