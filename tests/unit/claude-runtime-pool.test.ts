...
import type { RuntimePoolAccess } from '../../src/agent/runtime-pool.js';
...
test(`${factoryMode} reuses one real SDK query with fresh bindings and an actual child`, async t => {
    const { access } = accessFixture(); t.after(() => drain(access));
