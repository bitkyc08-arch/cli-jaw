...
import { describeGoalPauseGate } from './pause-gate.js';
...
export function buildGoalContinuation(): GoalContinuationResult {
    ...
    const prompt = [
        ...
        'For development goals, every phase gate and final completion must carry a documentation + implementation + verification evidence bundle. Documentation evidence = project-approved worklog or documentation path; implementation evidence = changed source/test paths or explicit no-code rationale; verification evidence = fresh command/test output.',
        '',
        ...
        '- Use explicit worklog/project-policy paths and decade numbering; keep private records external if required. Never invent a log directory.',
        '- Every phase needs a worklog or documentation update.',
        '',
...
'WORK-PHASE BOUNDARY: after C passes, run `cli-jaw orchestrate D --attest \'{"from":"C","to":"D","did":"<what you checked>","checkOutput":"<tsc/test tail>","exitCode":0}\'` to close the cycle (state returns to IDLE). If the goal objective still has remaining work-phases, run `cli-jaw orchestrate P` to start the next work-phase. Repeat until the objective is met.',
        ...
        ...(pauseGate.armed
            ? [
                ...
                `Your previous \`cli-jaw goal pause --agent --audit "<evidence>"\` was recorded as attempt ${pauseGate.attempts}/${pauseGate.requiredAttempts}. The goal is now in \`pause_gate_pending\`.`,
                ...
                '- project-approved working log entry with plan + outcome, changed source/test paths, fresh verification output.',
                '',
    ...
    return { shouldContinue: true, reason: pauseGate.reason ?? 'goal_active', prompt };
}
