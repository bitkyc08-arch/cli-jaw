...
`presentation.mode` accepts only activity/legacy; absence defaultsActivity for fresh/upgraded homes, explicitLegacy survives reload, and perCli.transport is independent. Only a sole own presentation patch skips fallback reset/singleton sync/JWC rewrite; keep serialization, persistence rollback and messaging dispatcher, with zero actual channel restart. Classic preference refresh uses the existing bounded reader and shared load/event generation; never replace native-request-bridge with the old Web donor. Manager Display owns singleflight, disabled/guarded edits, captured dirty acknowledgement and client/port epochs. Classic live Activity now consumes the canonical stream; cold history, Manager integration and TUI remain separate follow-ons. Keep bounded views, current-run final ownership and independent request controls.
