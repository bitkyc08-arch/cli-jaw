...
graph TD
    ...
    NEXT --> CHECK{"모든 agent phase 3 통과?"}
    RETRY --> CHECK
...
| ----------- | ------------------- | ---------------------- | -------------------------- |
| **Round 1** | Phase 1: 설계검증 ✅ | Phase 1: 설계검증 ✅    | Phase 1: 설계검증 ❌        |
| **Round 2** | Phase 2: 구현 ✅     | Phase 2: 구현 ❌        | Phase 1: 설계검증 재시도 ✅ |
| **Round 3** | Phase 3: 디버깅 ✅   | Phase 2: 구현 재시도 ✅ | Phase 2: 구현 ✅            |
| **결과**    | ✅ 완료              | ⚠️ Phase 3 미진입       | ⚠️ Phase 3 미진입           |

→ 유저에게: "A 완료, B·C는 구현까지 완료했으나 디버깅 미완. 이어서 할까요?"
...
const agentPhases = subtasks.map(st => ({
  ...
  task: st.task,
  domain: st.domain,
  currentPhase: 1,    // 1=설계검증, 2=구현, 3=디버깅
  completed: false,
  history: [],        // 각 라운드 결과 기록
}));
```
