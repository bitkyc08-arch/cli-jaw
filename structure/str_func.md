...
│   │   ├── spawn.ts          ← CLI spawn + ACP/Codex App/Pi RPC/AGY/Kiro plain text/log session capture/claude-e helper 분기 + v2 SQLite session resume + 큐 + 메모리 flush + 429 retry timer + isAgentBusy/isSteerInProgress + buildHistoryBlock compact cutoff + working_dir scoping + enqueue→processQueue race fix + QueueItem persistent DB queue + makeCleanEnv PATH augment (3284L)
...
│   │   │   └── process-kill.ts ← child process kill helper (169L)
...
│   │   ├── pi-runtime.ts     ← Pi profile 정규화 + isolated `PI_CODING_AGENT_DIR` models/settings 생성 + `pi --offline --list-models` discovery + `pi --mode rpc` JSONL parser/spawner (792L) ✨
...
│   │   ├── codex-app-client.ts ← Codex App stdio server client (1426L)
...
│   │   ├── acp-client.ts     ← Copilot ACP JSON-RPC 클라이언트 (392L)
...
│   │   ├── bot.ts            ← Slack 봇 lifecycle + envelope routing + orchestrate 경로 + queued-result waiter (833L)
...
│   │   ├── thread-tracker.ts ← 참여 스레드 영속 추적 (mention/봇응답 마킹, 캡드 셋, 무멘션 스레드 연속 대화 게이트 지원) (194L)
...
│   │   ├── agent.ts          ← ToolEntry, SpawnContext, SpawnResult 인터페이스 (172L)
...
│       ├── init.ts           ← 초기화 마법사 + --safe/--dry-run + --help (502L)
...
│       ├── doctor.ts         ← 진단 (다중 체크 + claude-i helper/underlying claude + headless 감지, --json) (974L)
