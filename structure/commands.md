...
| `chat` | `bin/commands/chat.ts` | `process.argv.slice(3)`를 TUI로 전달. 기본/`--raw`/`--simple` 모드. TUI transport는 `bin/commands/tui/channel.ts`에서 SSE-first inbound(`GET /api/events`) + legacy WS fallback(pre-X-01 server only)을 제공하고, outbound는 REST `POST /api/message` / `POST /api/stop`을 사용. Interactive native Activity는 서버 snapshot identity를 사용하며 `/settings`에서 Activity(default)/Legacy 표시를 전환한다. Ctrl+O는 현재 Activity 상세를 여닫고 최종응답은 별도 본문에 남긴다. Fullscreen의 F6 기록 창은 방향키·Enter·PgUp/PgDn으로 탐색하고 R로 다시 읽으며 N으로 다음 실행 목록을 읽는다. |
