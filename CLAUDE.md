...
- Agent pause is a two-tap audited gate. After the first `--agent --audit` attempt, the goal remains persisted as `active` but status/API surfaces expose derived `pauseGate: { armed: true, reason: "pause_gate_pending" }`; while armed, automatic goal continuation is suppressed and the audit turn may emit `goal_pause_gate_pending` without scheduling another kick. A second audited pause pauses the goal; a productive checkpoint clears the gate.
...
- Telegram Hub: forum topic routing via Dashboard `/api/dashboard/telegram-hub`; hub commands `/setthread` `/threads` `/hubhelp`; per-topic `model`/`systemPrompt` overrides (P4). One bot token → one long-poller. See `structure/telegram.md`.
...
- AGY (`agy`) is a top-level runtime, not an `ai-e` provider. It runs in print mode through `agy -p` with optional `--model` when not `default`, captures print-mode session ids from a per-run `--log-file`, resumes exact saved sessions with `--conversation <id>`, exposes no per-run `--effort` flag, checks auth at run time, and uses plain-text stdout rather than NDJSON parsing.
