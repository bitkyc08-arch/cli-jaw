...
`src/messaging/outbound-lifecycle.ts` owns LIVE outbound turns, which is the opposite direction from `QueueNoticeRegistry`: that one holds cleanups shutdown STARTS, this one holds sends shutdown must STOP. All three paths register (normal, queued, forwarder) — wiring only the queued branch leaves the ordinary reply and the standing forwarder invisible to shutdown. `track()` lets a drain WAIT for nearly-finished work instead of cancelling at time zero; the abort after the deadline is what actually stops it. `end()` releases without aborting. Abort classification splits shutdown cancels from vendor faults so a cancelled send is not logged as a delivery failure. Outbox contract for an aborted send: `sending` -> `ambiguous` (the bytes left, nobody can confirm), `pending` -> `definitive_failed`, terminal rows untouched (#417).
Cancellation reaches the vendor per channel: Telegram through `createIpv4Fetch` (already destroyed the request on abort; the caller now supplies a signal), Discord through `rest-scheduler` (honours a signal while queued behind a rate limit, which `channel.send()` cannot), Slack through `slackApi` plus an abortable rate-limit wait and a signal threaded through all three upload steps. Every chunk loop re-checks between chunks: an abort reaches the request in flight, not the loop that would send the next one.
