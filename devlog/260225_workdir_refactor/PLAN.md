...
### Why No Stale Cleanup Is Needed

When workingDir moves to `~/.cli-jaw`, `AGENTS.md` is generated inside a hidden
dotfile directory. CLI tools (Claude, Codex, Copilot) walk **up** the directory tree
from cwd to `/`, but they don't descend **into** dotfile directories. So even if a
user later changes workingDir to something else, the old `~/.cli-jaw/AGENTS.md`
won't pollute other sessions — it's safely isolated.

**One-time manual cleanup:** Users should delete `~/AGENTS.md` and `~/CLAUDE.md`
after upgrading, since those were generated by the old `~/` default. This could be
mentioned in release notes or done by `cli-jaw init` with a prompt.
...
- If user switches back, old `AGENTS.md` is still there and gets overwritten

**Rationale:**
- Simple, no risk of data loss
- Hidden dotfile directories don't cause CLI pollution
- Users running multiple workspaces can switch freely
- Session memory (`~/.claude/projects/`) is Claude CLI's own concern, not ours
- Our persistent data (jaw.db, MEMORY.md) lives at `~/.cli-jaw/` regardless of workingDir

### What We Don't Need To Do

| Item | Needed? | Why |
|------|---------|-----|
| Migrate old AGENTS.md on workingDir change | ❌ No | New one auto-generated; old one harmless in dotfile dir |
| Update `~/CLAUDE.md` symlink on change | ❌ No | Symlink was a `~/` era artifact; with dotfile dir, not needed |
| Track previous workingDir for cleanup | ❌ No | Stale files in dotfile dirs don't pollute |
| Migrate session memory | ❌ No | Claude CLI manages `~/.claude/projects/` independently |
...
Only 2 patches needed. Total: **3 files, 3 lines changed.**
...
- **Existing users**: keep their persisted `settings.workingDir` (no auto-change)
