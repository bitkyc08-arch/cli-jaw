...
- **Revising** Korean prose that already exists — 윤문, 다듬어, 자연스럽게, 교정, AI투, 번역투 → `jaw-dev-write`. This also applies to Korean you are about to send: run its protocol on your own draft before output.
- **Composing** an answer for a person — explaining, reporting, teaching, replying → `jaw-dev-speech`. It owns explanation order, audience calibration, and hedging discipline.
