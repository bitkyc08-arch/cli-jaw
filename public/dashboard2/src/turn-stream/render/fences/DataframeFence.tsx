...
export function DataframeFence({ spec }: { spec: DataframeSpec }): ReactElement {
    const ports = useRenderActionPorts(); const [filter, setFilter] = useState(''); const [copyError, setCopyError] = useState(''); const [sort, setSort] = useState<{ column: number; direction: 1 | -1 } | null>(null); const [page, setPage] = useState(0);
    ...
    return <section className="dataframe-block"><header>{spec.title}</header><input aria-label="Filter rows" value={filter} onChange={e => { setFilter(e.target.value); setPage(0); }} /><div className="d2-table-wrapper"><table><thead><tr>{spec.columns.map((column, i) => <th key={column}><button type="button" onClick={() => setSort(current => current?.column === i ? { column: i, direction: current.direction === 1 ? -1 : 1 } : { column: i, direction: 1 })}>{column}</button></th>)}</tr></thead><tbody>{visible.map(({ row, index }) => <tr key={index}>{row.map((cell, i) => <td key={i}><button type="button" onClick={() => { void copyWithFeedback((t) => ports.copyText(t), cell).then((outcome) => { if (outcome.ok) { ports.announce('Cell copied'); } else { setCopyError(outcome.message); } }); }}>{cell}</button></td>)}</tr>)}</tbody></table></div><footer><button disabled={safePage === 0} onClick={() => setPage(p => Math.max(0, p - 1))}>Previous</button><span>{safePage + 1}/{pages}</span><button disabled={safePage + 1 >= pages} onClick={() => setPage(p => p + 1)}>Next</button>{copyError ? <div role="alert">{copyError}</div> : null}</footer></section>;
}
