...
export function buildSearchSteerPrompt(options: SearchWorkflowPromptOptions): string {
    ...
    return [
        ...
        '',
        'Command boundary: /search owns this command-specific report shape. Canonical provider policy, source-class routing, and evidence-status rules come from the active search skill.',
        '',
        'Required workflow:',
        ...
        '8. Route known source classes through the active search skill reader policy, then fetch or open at least one original/primary source before marking evidence sufficient.',
