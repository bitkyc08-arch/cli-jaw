...
- Embedded Browser agent surface (030, v1–v5): "Share with Agent" relays shared pages into the SELECTED instance's runtime-context. Agent endpoints via a renderer-relayed command queue: `POST …/<targetId>/screenshot` (PNG temp-file path), `POST …/<targetId>/snapshot` (bounded accessibility tree), and `POST …/<targetId>/act` (click/type/scroll/key). `act` requires a SECOND opt-in beyond share — the per-tab "Allow agent actions" toggle (`actionsEnabled`) — and validates payload bounds + re-checks the current URL policy in main. Element inspect/actions use `webContents.debugger` CDP with ONLY DOM/Overlay/Input/Accessibility domains (native element-box highlight via `Overlay.setInspectMode`); the Runtime domain / page-side script evaluation is never enabled. Page titles/urls are sanitized + JSON-delimited before entering agent context.
