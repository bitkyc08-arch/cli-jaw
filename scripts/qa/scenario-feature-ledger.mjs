...
export const FEATURE_SCENARIOS = [
    ...
    // ── hover dock: agents nested matrix ────────────────────────────────────
    {
        ...
        ...DOCK,
        dockTab: '에이전트',
        levers: { settingsConfig: { holdInstanceSettings: true } },
        selector: '.hover-dock-body[data-dock-tab="agents"] .dock-loading',
        expected: '로딩 중',
    },
    {
    ...
    },
    {
        ...
        ...DOCK,
        dockTab: '에이전트',
        levers: { settingsConfig: { instanceSaveStatus: 500 } },
        actions: [{ kind: 'select', selector: '.hover-dock-body[data-dock-tab="agents"] select', value: 'high', nth: 2 }],
        expectRequests: [{ method: 'PUT', path: '/i/3506/api/settings', count: 1 }],
        selector: '.hover-dock-body[data-dock-tab="agents"] .dock-error',
        expected: '저장 실패',
    },
    {
        id: 'dock-agents-employees-empty',
        reachability: 'integration',
        axis: 'empty', target: 'List',
        why: 'no employees are configured',
        ...DOCK,
        dockTab: '에이전트',
        levers: { employees: { employees: [] } },
        selector: '.hover-dock-body[data-dock-tab="agents"]',
    ...
    {
        id: 'dock-agents-employees-delete-error',
        reachability: 'integration',
        axis: 'error', target: 'Alert',
        why: 'deleting an employee was rejected',
        ...DOCK,
        dockTab: '에이전트',
        levers: { employees: { employees: [{ id: 'e1', name: 'Ada', cli: 'codex', model: null, role: null, status: 'idle' }] }, hoverDock: { mutationStatus: 500 } },
        actions: [{ kind: 'click', selector: '.hover-dock-body[data-dock-tab="agents"] .dock-mini-btn[title="삭제"]' }],
        expectRequests: [{ method: 'DELETE', path: '/i/3506/api/employees/e1', count: 1 }],
        selector: '.hover-dock-body[data-dock-tab="agents"]',
        requires: '.dock-emp',
    },
    {
        id: 'dock-agents-flush-collapsed',
        reachability: 'integration',
        axis: 'ready', target: 'Section',
        why: 'the flush section starts collapsed, showing only its header',
        ...DOCK,
        dockTab: '에이전트',
        levers: { hoverDock: {} },
    ...
    {
        id: 'dock-skills-ready',
        reachability: 'integration',
        axis: 'ready', target: 'List',
        why: 'skills render with enabled styling and a count',
        ...DOCK,
        dockTab: '스킬',
        levers: { hoverDock: { skills: [{ id: 's1', name: 'review', enabled: true, description: 'review code' }, { id: 's2', name: 'plan', enabled: false, description: 'plan work' }] } },
        selector: '.hover-dock-body[data-dock-tab="skills"] .dock-skill.is-enabled',
        expected: 'review',
    },
    {
        id: 'dock-skills-enable',
        reachability: 'integration',
        axis: 'action', target: 'Control',
        why: 'enabling a disabled skill POSTs to enable',
        ...DOCK,
        dockTab: '스킬',
        levers: { hoverDock: { skills: [{ id: 's2', name: 'plan', enabled: false, description: 'plan work' }] } },
        actions: [{ kind: 'click', selector: '.hover-dock-body[data-dock-tab="skills"] .dock-switch' }],
        expectRequests: [{ method: 'POST', path: '/i/3506/api/skills/enable', count: 1 }],
        selector: '.hover-dock-body[data-dock-tab="skills"]',
    },

    // ── hover dock: settings sections (non-mutation + mutation) ─────────────
    {
        id: 'dock-settings-prompt-save',
        reachability: 'integration',
        axis: 'action', target: 'Control',
    ...
    },
    {
        ...
        reachability: 'integration',
        axis: 'prerequisite', target: 'Control',
        why: 'with no MCP bundle, install is disabled',
        ...DOCK,
        dockTab: '설정',
        levers: { hoverDock: { mcp: { ok: true, data: { servers: {} } } } },
        selector: '.hover-dock-body[data-dock-tab="settings"] button[disabled]',
        expected: 'Install bundle (0)',
    },
    {
    ...
    },
    {
        id: 'dock-close-esc',
        reachability: 'integration',
        axis: 'action', target: 'Panel',
        why: 'Escape closes the dock',
        ...DOCK,
        levers: { hoverDock: {} },
        actions: [{ kind: 'press', key: 'Escape' }],
    ...
    {
        id: 'sidebar-instances-empty',
        reachability: 'integration',
        axis: 'empty', target: 'StatePanel',
        why: 'no instances are running',
        ...SIDEBAR,
        preseedSidebar: { instances: [] },
        selector: '.d2-sidebar-v4 .d2-inline-state',
        expected: 'No instances found',
    },
    {
        id: 'sidebar-sessions-loading',
        reachability: 'integration',
        axis: 'loading', target: 'StatePanel',
        why: 'an instance session list is in flight',
        ...SIDEBAR,
        levers: { sidebar: { holdChatSessions: true } },
        actions: [{ kind: 'click', selector: '.d2-instance-row .d2-instance-main' }],
        selector: '.d2-sidebar-v4 .d2-inline-state, .d2-sidebar-v4 .d2-spinner',
        requires: '.d2-sidebar-v4',
    },
    {
        ...
        reachability: 'integration',
        axis: 'empty', target: 'StatePanel',
        why: 'an instance has no sessions',
        ...SIDEBAR,
        levers: { sidebar: { chatSessions: { 3506: { sessions: [], active: null } } } },
        actions: [{ kind: 'click', selector: '.d2-instance-row .d2-instance-main' }],
        selector: '.d2-sidebar-v4',
        requires: '.d2-inline-state, .d2-sidebar-empty',
    },

    // ── sidebar: the jwc async surface (wp10 t2, third surface) ─────────────
    {
        id: 'sidebar-jwc-loading',
        reachability: 'integration',
        axis: 'loading', target: 'StatePanel',
        why: 'the jwc capability probe is in flight',
        ...SIDEBAR,
        levers: { code: { holdCapability: true } },
        actions: [{ kind: 'click', selector: '.d2-mode-button[role="tab"]:has-text("jwc")' }],
        selector: '.d2-sidebar-v4 .d2-inline-state',
        expected: 'Loading Code sessions',
    },
    {
        id: 'sidebar-jwc-temporarily-unavailable',
        reachability: 'integration',
        axis: 'error', target: 'StatePanel',
        why: 'the jwc runtime is temporarily unavailable (default probe answer)',
        ...SIDEBAR,
        levers: { code: { capability: { available: false, reason: 'temporarily_unavailable' } } },
        actions: [{ kind: 'click', selector: '.d2-mode-button[role="tab"]:has-text("jwc")' }],
        selector: '.d2-sidebar-empty[data-jwc-state="temporarily_unavailable"]',
        expected: 'Code runtime temporarily unavailable',
    },
    {
        id: 'sidebar-jwc-missing-binary',
        reachability: 'integration',
        axis: 'prerequisite', target: 'StatePanel',
        why: 'jwc is not installed',
        ...SIDEBAR,
        levers: { code: { capability: { available: false, reason: 'missing_binary' } } },
        actions: [{ kind: 'click', selector: '.d2-mode-button[role="tab"]:has-text("jwc")' }],
        selector: '.d2-sidebar-empty[data-jwc-state="missing_binary"]',
        expected: 'jwc is not installed',
    },
    {
        id: 'sidebar-jwc-error',
        reachability: 'integration',
        axis: 'error', target: 'StatePanel',
        why: 'the stored-session read failed after capability ok (the only path to jwc error)',
        ...SIDEBAR,
        levers: { code: { capability: { available: true, reason: 'ok' }, storedStatus: 500 } },
        actions: [{ kind: 'click', selector: '.d2-mode-button[role="tab"]:has-text("jwc")' }],
        selector: '.d2-sidebar-empty[data-jwc-state="error"]',
        expected: 'Failed to check Code availability',
    },
    {
        id: 'sidebar-jwc-conversations',
        reachability: 'integration',
        axis: 'ready', target: 'List',
        why: 'jwc conversations render when capability is ok',
        ...SIDEBAR,
        levers: { code: {
            capability: { available: true, reason: 'ok' },
            storedSessions: [{ sessionId: 'jwc-1', cwd: '/tmp/wp4-e2e', title: 'wp10 jwc convo', updatedAt: '2026-07-26T00:00:00.000Z', messageCount: 2 }],
        } },
        actions: [{ kind: 'click', selector: '.d2-mode-button[role="tab"]:has-text("jwc")' }],
        selector: '.d2-sidebar-v4 .d2-jwc-conv-row',
        expected: 'wp10 jwc convo',
    },
    {
    ...
    },
    {
        ...
        actions: [
            ...
            // expand it, then click its session row.
            { kind: 'click', selector: '.d2-instance-row[data-instance-port="3506"] .d2-instance-main' },
            { kind: 'click', selector: '.d2-instance-node:has([data-instance-port="3506"]) .d2-session-row' },
