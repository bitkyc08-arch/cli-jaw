...
Print-mode providers also record accepted text, reasoning and tool activity without changing their existing answer selection. Untagged text remains unclassified; the existing lifecycle supplies the application-final answer. Tool status/detail converges across live and saved views, and partially retained history keeps its omission notice. Activity recording failure does not add a retry or channel send. Full Activity rendering remains a separate rollout.

Display preferences now use `presentation.mode`: `activity` is the default for fresh and upgraded settings without a choice; explicit `legacy` is preserved. Manager → Display provides the reversible choice, independently of runtime transport and permissions. Classic applies preference updates without reopening the native request panel or triggering runtime migration prompts. This preference checkpoint does not yet replace the Legacy transcript renderer; full Activity rendering follows separately.
