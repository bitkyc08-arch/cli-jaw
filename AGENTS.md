...
- Claude native main: one optional SDK query, shared runtime pool/host and existing lifecycle own sequential turns, immutable terminal claims, hard Stop and interrupted MESSAGE before exit-settle. No in-band steer is advertised; the default policy retains kill/resume salvage. Public native workers and approval/question/image/child capabilities remain disabled until their parity gate. Pre-start failures and pre-acquisition Stop start one cached fallback projection before compatibility output; late exceptional trace close uses `onlyIfRunning` so an already finished header is preserved.
