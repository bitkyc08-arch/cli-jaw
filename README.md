...
Claude native main uses the optional pinned Agent SDK with persistent sequential sessions. Stop closes the query; the default mid-run steer policy uses kill/resume with interrupted context, while follow-up mode queues. Worker, approval/question, image and subagent activation remain separate capabilities and are not enabled by this main-only integration. An unavailable SDK fails the explicit native request without falling back to print.
