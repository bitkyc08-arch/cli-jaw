...
Codex app-server now attaches the projection after its existing lane/turn owner gate and legacy consumer. Host-only, foreign-thread and stale-turn notifications remain raw-only. Main multiplex ON/OFF and employee paths capture jaw identity once; lifecycle supplies the terminal rather than a tool/message completion guess. One projection failure emits agent_runtime_gap and disables later canonical writes for that run; ordinary final delivery and salvage remain independent. Pi integration follows separately. No runtime/default selection is changed by this projection.
