...
| Electron self-contained sidecar release | `scripts/bundle-sidecar.sh`, `scripts/check-electron-sidecar-jwc.cjs`, `scripts/check-app-icon-assets.cjs`, `electron/electron-builder.yml`, `.github/workflows/desktop-release.yml`, `electron/src/main/lib/jaw-spawn.ts`, `electron/src/main/lib/install-cli.ts` | Packaged desktop builds include a Node.js 22.16 sidecar under `extraResources/server`, install `jawcode` from a packed tarball so `bin/jwc` and runtime deps (`json5`, `strip-ansi`, `markit-ai`) are present, prefer bundled `bin/jaw` before global `jaw`, fail CLI install on incomplete sidecars, verify staging/final macOS `.app` sidecars plus packaged app icons, and build macOS arm64 DMG/ZIP, Windows x64 NSIS/ZIP, and Linux AppImage from GitHub Actions release/manual dispatch. |
