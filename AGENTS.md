...
- Claude native main/workers: `claude-runtime-pool.ts` and `claude-runtime-run.ts` use the existing pool, shared runner and lifecycle with safe/auto permissions; print remains the default. Agent/Task calls require explicit `run_in_background:false`; background work is unsupported. Native steer uses kill/salvage. `runtime/claude-run-controls.ts` retains worker identity and shutdown accounting through logical settlement and proven physical cleanup; failed close keeps the fence and allocated temporary directory. Preserve captured trace/worker ownership and claim-before-delivery finalization. See `structure/runtime-integration.md`.
