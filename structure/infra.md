...
`src/messaging/queue-notice-store.ts` owns `queue_notices`. The row is reserved BEFORE the notice is posted and carries the platform message id once it lands; the crash window therefore falls the harmless way (a record with no id restores to nothing). It is not the queue payload, which is deleted when the item starts running. `queue-notice-restore.ts` always rewrites as `expired`, never deletes — a restart means the turn never delivered through its original handle, and the boot drain posts the answer separately. Restore runs BEFORE `drainRecoveredQueue()` so a fresh answer never lands under a notice still claiming to wait. A record whose transport cannot be built yet is kept (temporary); one the vendor rejects is dropped (it will not succeed next boot either). Every channel-side store call is best-effort: a durable write is a convenience for the next boot and must not fail the turn in flight (#418).
