...
| Release gates | `scripts/release-gates.mjs`, `package.json` | `gate:all` runs all 21 named gates (`typecheck`, `tests`, `truth-table-fresh`, `mcp-scope-frozen`, `no-experimental-in-readme-ready-section`, `no-cloud-claims`, `observe-actions-fixtures`, `observation-bundle-fixtures`, `browser-primitives-catalog`, `action-memory-safe-replay`, `model-adapter-frozen`, `workflow-common-layer-no-sdk`, `doc-drift`, `strict-baseline`, `redaction-sinks`, `electron-version`, `gate-docs`, `sidecar-prune-safety`, `native-load`, `sidecar-smoke`, `install-integrity`); each is npm-addressable as `gate:<name>`. This row is itself enforced by `gate:gate-docs`. |
