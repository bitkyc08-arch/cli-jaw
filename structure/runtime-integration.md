...
This session is not yet wired into native selection, main/worker pools or Activity UI. Internal tools/toolOutput capabilities are true; approval/question/image/subagent capabilities remain false and pending tool decisions are denied. Foreground-only hooks reject unsupported background modes without promising an OS sandbox. Claude print and claude-e remain unchanged. SDK authentication follows the official API/cloud setup; no claude.ai login flow, credential copying or subscription entitlement is added. Current integration qualification distinguishes real pinned SDK/owned simulated CLI from historical provider evidence and does not claim a fresh live-provider run.
