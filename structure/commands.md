...
> 모델/CLI 선택은 `registry.ts` 단일 소스를 따른다. `jwc`는 2026-07-11 Jaw 채팅 런타임에서 퇴역했다 (Code 모드 ACP host와 `jaw jwc` 설치 헬퍼는 존속, devlog 012 참조). 현재 registry 런타임은 `pi`, `agy`, `ai-e`, `claude`, `claude-e`, `codex`, `codex-app`, `cursor`, `grok`, `kiro-code`, `opencode`, `copilot` 12개다.
