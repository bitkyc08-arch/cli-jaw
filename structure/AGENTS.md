...
- `src/cli/commands.ts` / `src/cli/handlers/session-handlers.ts` / `src/core/chat-sessions.ts`: slash registry currently includes 51 commands. Keep quoted-argument `tokenizeArgs()`, Levenshtein unknown-command recovery, `/goalplan`, `/gd`, `/review`, `/search`, `/task`, and `/fork` reflected in `commands.md`.
