...
import { workerApiUrl } from '../../detail/detail-client.ts';
...
export function LinkPreviewCard({ data, sourceUrl, identity }: { data: PreviewMetadata; sourceUrl: string; identity: RenderIdentity }): ReactElement {
    ...
    return <a className="d2-link-preview" href={href} target="_blank" rel="noopener noreferrer">
        {data.image && workerPort !== null ? <img src={workerApiUrl(workerPort, `/api/link-preview/image?url=${encodeURIComponent(data.image)}`)} alt="" loading="lazy" referrerPolicy="no-referrer" onLoad={invalidate} onError={invalidate} /> : null}
...
export function LinkPreviewLayer({ enabled, host, revision, identity }: { enabled: boolean; host: HTMLElement | null; revision: string; identity: RenderIdentity }): ReactElement | null {
    ...
    useEffect(() => {
        if (!enabled || !host || workerPort === null) return;
        ...
        const load = async ({ anchor, url }: { anchor: HTMLAnchorElement; url: string }) => {
            const cacheKey = `${workerPort}:${url}`;
            const cached = cache.get(cacheKey); if (cached !== undefined) { if (cached) setCards(current => current.some(x => x.anchor === anchor) ? current : [...current, { anchor, url, data: cached }]); return; }
            const controller = new AbortController(); controllers.add(controller);
            try { const response = await fetch(workerApiUrl(workerPort, `/api/link-preview?url=${encodeURIComponent(url)}`), { signal: controller.signal }); const json = response.ok && response.status !== 204 ? await response.json() as { ok?: boolean; data?: PreviewMetadata } : null; const data = json?.ok && json.data ? json.data : null; cache.set(cacheKey, data); while (cache.size > MAX_CACHE) cache.delete(cache.keys().next().value!); if (data && anchor.isConnected) setCards(current => [...current, { anchor, url, data }]); } catch (error) { if (!(error instanceof DOMException && error.name === 'AbortError')) cache.set(cacheKey, null); } finally { controllers.delete(controller); }
        };
    ...
    }, [enabled, host, revision, workerPort]);
    if (!enabled || workerPort === null) return null;
    return <>{cards.map(({ anchor, url, data }) => anchor.isConnected ? <LinkPreviewPortal key={`${workerPort}:${url}`} anchor={anchor} card={<LinkPreviewCard data={data} sourceUrl={url} identity={identity} />} /> : null)}</>;
}
