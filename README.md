...
Codex app-server and Pi RPC also record versioned, redacted runtime events independently of channel delivery. Native terminal handling keeps empty final answers distinct from live previews; Slack/Telegram/Discord retain their existing final-response, acknowledgement and queue-notice flow. Rich terminal chat defaults to Activity for versioned runtime events, with Legacy presentation available in `/settings`. Ctrl+O toggles the current Activity details; F6 opens retained records in fullscreen chat. Full final answers remain separate from bounded previews, and raw NDJSON stays unchanged.
