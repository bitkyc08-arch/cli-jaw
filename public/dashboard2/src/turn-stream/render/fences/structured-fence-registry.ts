...
export interface StructuredFenceSuccess { kind: 'component'; fenceKind: StructuredFenceKind; spec: StructuredFenceSpec; component: ComponentType<{ spec: never }> }
...
type Entry = { parse(raw: string): StructuredFenceSpec | null; component: ComponentType<{ spec: never }> };
const entry = <T,>(parse: (raw: string) => T | null, component: ComponentType<{ spec: T }>): Entry => ({ parse: parse as (raw: string) => StructuredFenceSpec | null, component: component as ComponentType<{ spec: never }> });
