...
- Forward transitions (P→A→B→C→D) require an EVIDENCE attestation — a real \`cli-jaw orchestrate\` command, not narration like "현재는 B입니다": \`cli-jaw orchestrate B --attest '{"from":"A","to":"B","did":"<what you actually did>"}'\`. C→D additionally needs a pasted check tail: \`--attest '{"from":"C","to":"D","did":"...","checkOutput":"<tsc/test tail>","exitCode":0}'\`. Saying a phase is done without running the command does nothing — the state machine only moves on the command.
