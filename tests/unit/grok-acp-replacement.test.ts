...
test('stop before the first queued start performs no IO', async () => {
    const f = fixture();
    const first = f.control.first({ text: 'A' }); const rejected = assert.rejects(first, /grok_control_stopped/);
...
test('replacement is reentrant during a replacement registration without duplicating starts', async () => {
    const f = fixture(); await f.ready(); let c: ReturnType<GrokReplacement['replace']> | undefined;
...
test('cancel failure while dispatch is held rejects both operations with one fatal retirement', async () => {
    ...
    assert.equal(f.retired.length, 1); assert.equal((f.retired[0] as GrokReplacementError).stage, 'cancel');
    assert.equal(f.retired[0]!.cause, cause); assert.equal(f.control.isStopped, true);
    await assert.rejects(f.control.replace({ text: 'C' }), GrokReplacementError); assert.equal(f.starts.length, 1);
});

test('replacement dispatch failure retires and rejects rather than returning a retryable no-start', async () => {
    const f = fixture(); await f.ready();
    const replacement = f.control.replace({ text: 'B' }); const rejected = assert.rejects(replacement, GrokReplacementError);
    f.drain(); await f.waitFor(() => f.starts.length === 2); f.starts[1]!.written.reject(new Error('write failed'));
    await rejected; assert.equal(f.retired.length, 1); assert.equal((f.retired[0] as GrokReplacementError).stage, 'dispatch');
    await assert.rejects(f.control.replace({ text: 'C' }), GrokReplacementError); assert.equal(f.starts.length, 2);
});

test('first failure keeps the fatal type even if retirement itself throws', async () => {
    const f = fixture(); f.throwRetirement();
    const first = f.control.first({ text: 'A' }); const rejected = assert.rejects(first, GrokReplacementError);
...
test('before-first replacement and duplicate first do not disturb the original attempt', async () => {
    ...
    await assert.rejects(f.control.first({ text: 'duplicate' }), /grok_control_already_started/);
