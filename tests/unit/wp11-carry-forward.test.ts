...
test('CF-2: a completed elicitation hydrates from the registry, not fresh state', () => {
    ...
    assert.equal(getElicitationCompletion(identity.scopeKey, key), null, 'nothing before completion');

    setElicitationCompletion(identity.scopeKey, key, { answers: [{ questionId: 'q1', question: 'Pick one', skipped: false, values: ['a'], labels: ['A'] }] });
    const hydrated = getElicitationCompletion(identity.scopeKey, key);
    ...
    assert.equal(getElicitationCompletion(identity.scopeKey, other), null, 'a different elicitation has no completion');
    resetElicitationRegistry();
});
