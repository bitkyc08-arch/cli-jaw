...
| Grok CLI runtime | `src/cli/registry.ts`, `src/agent/args.ts`, `src/agent/events.ts`, `src/routes/quota.ts`, `public/assets/providers/grok*.svg` | `grok-build` is a standard CLI runtime using `grok -p ... --output-format streaming-json`; effort/system-prompt flags are disabled for `grok-build`; `/api/quota.grok` reads eligible xAI OIDC/external credentials from `~/.grok/auth.json` and prefers Grok Build `GET /v1/billing?format=credits` weekly data before falling back to legacy `cli-chat-proxy.grok.com/v1/billing`. Browser `vendor=grok` remains a separate web-AI surface. |
